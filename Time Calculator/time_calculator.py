def add_time(start, duration, start_date=False):
    ...
    time_total = (start_hours * 60) + start_minutes + \
    ...
    if days_later == 0:
        ...
        new_time = "{0}:{1} {2} (next day)".format(
            new_hours, new_minutes, new_meridiem)
    else:
        new_time = "{0}:{1} {2} ({3} days later)".format(
            new_hours, new_minutes, new_meridiem, days_later)
    ...
    if start_date:
        ...
        if days_later == 0:
            new_time = "{0}:{1} {2}, {3}".format(
                new_hours, new_minutes, new_meridiem, new_start_date)
        elif days_later == 1:
            new_time = "{0}:{1} {2}, {3} (next day)".format(
                new_hours, new_minutes, new_meridiem, new_start_date)
        else:
            new_time = "{0}:{1} {2}, {3} ({4} days later)".format(
                new_hours, new_minutes, new_meridiem, new_start_date, days_later)
